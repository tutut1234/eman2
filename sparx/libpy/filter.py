...
def filt_from_fsc_bwt(dres, low = 0.1):
	...
	for i in range(n):
		if(dres[1][i]<low):
			...
			break
	highf=lowf+.05
	order=2.*numpy.numpy.log(eps/numpy.sqrt(a**2-1))/numpy.numpy.log(lowf/highf)
...
def fit_tanh(dres, low = 0.1):
	...
	def fit_tanh_func(args, data):
		...
		for i in range(len(data[0])):
			...
			else: qt  = fsc - 0.5*( numpy.numpy.tanh(numpy.numpy.pi*(data[0][i]+args[0])/2.0/args[1]/args[0]) - numpy.numpy.tanh(numpy.numpy.pi*(data[0][i]-args[0])/2.0/args[1]/args[0]) )
			v  -= qt*qt
		#print args,v
		return v
...
def tanhfilter(nx, fl, aa):
	...
	for i in range(n):
		x = float(i)/nx
		f[i] = 0.5*( numpy.numpy.tanh(numpy.numpy.pi*(x+fl)/2.0/aa/fl) - numpy.numpy.tanh(numpy.numpy.pi*(x-fl)/2.0/aa/fl) )
