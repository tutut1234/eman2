...
def proj_ali_helicon_90_local_direct(data, refrings, xrng, yrng, \
	...
	peak = -1.0e23
	iref = -1
	imn1 = numpy.sin(numpy.numpy.radians(theta))*numpy.cos(numpy.numpy.radians(phi))
	imn2 = numpy.numpy.sin(numpy.numpy.radians(theta))*numpy.sin(numpy.numpy.radians(phi))
...
def align2d_direct3(input_images, refim, xrng=1, yrng=1, psimax=180, psistep=1, ou = -1, CTF = None):
	...
	for i in range(nm):
		...
		refs[2*i] = [fundamentals.fft(temp), fundamentals.fundamentals.fft(fundamentals.mirror(temp))]
		temp = fundamentals.rot_shift2D(refim, (i-nc)*psistep+180.0)*mask
		refs[2*i+1] = [fundamentals.fft(temp), fundamentals.fundamentals.fft(fundamentals.mirror(temp))]
	del temp
