		...
		for ix in range(nx):
			...
			for iy in range(nx):
				...
				if( r2 < nr2 ):
					...
					try:
						u = numpy.numpy.sqrt( dfa/defc ) * numpy.numpy.sqrt(r2)
						iu = int(u)
						du = u - iu
		...
		for ix in range(nx):
			...
			for iy in range(nx):
				...
				if( r2 < nr2 ):
					...
					try:
						u = numpy.numpy.sqrt( Cst*(defc - numpy.numpy.sqrt( defc**2 + Cst**2*lam**4*s**4 - 2*Cst*lam**2*s**2*dfa)))/(Cst*lam) * nc*2*Pixel_size
						iu = int(u)
						du = u - iu
						lr[iu]    += (1.0-du)*img.get_value_at(ix,iy)
						lr[iu+1]  +=       du*img.get_value_at(ix,iy)
...
def defocus_L2_euc(v1,v2, ist,istp):
	...
	if dis    <= 0:		global_def.ERROR("bad fitting, change options settings and try again  ", "defocus_L2_euc", 0)
		...
		res = numpy.numpy.sqrt(dis)/numpy.sqrt(pw_sum)/tfeq	
		return res
...
#	for i, v in enumerate(flist):
		ncount   += 1
		defocus = defocus_get(fnam_roo, volt, Pixel_size, Cs, wgh, istart, istop, docf, skip, round_off, nr1, nr2)
		dis = None # Markus 2018/11/20 - To resolve unreferenced variable dis
		if(defocus > dz_max):
			while(nr1 <= 7 or nr2 <= 7):
				nr1 += 1
				nr2 += 1
				defocus = defocus_get(fnam_roo, volt, Pixel_size, Cs, wgh,istart, istop, docf,skip, round_off, nr1, nr2)
				if(print_screen[0] == "p" or print_screen[0] == "P" ): print("defocus",defocus,"Euclidean inspect.distance",dis,"starting feq",istart,"stop freq",istop,"P R E", nr1,"P R B", nr2)
				if(defocus<dz_max): break
		if(defocus > dz_max):
			while(nr1 >= 2 and nr2 >= 2):
				nr1 -= 1
				nr2 -= 1
				defocus = defocus_get(fnam_roo, volt,Pixel_size, Cs, wgh, istart, istop, docf, skip, round_off, nr1, nr2)
				if(print_sreen[0] == "p" or print_screen=="P"): print("defocus",defocus,"Euclidean inspect.distance",dis,"starting feq",istart,"stop freq",istop,"P R E", nr1,"P R B", nr2)
				if(defocus < dz_max): break
		if(defocus > dz_max):
			while(istart > istop):
				nr1    =  5
				...
				istart -=.5
				defocus = defocus_get(fnam_roo, volt, Pixel_size, Cs, wgh, istart, istop, docf,skip, round_off, nr1, nr2)
				if(print_screen[0] == "p" or print_screen == "P"): print("defocus",defocus,"Euclidean inspect.distance",dis,"starting feq",istart,"stop freq",istop,"P R E", nr1,"P R B", nr2)
				if(defocus < dz_max): break
		if(defocus > dz_max):
			while(istart > istop):
				nr1     = 5										    	
...
def residuals_lsq_peak(p,y,x,c):
	...
	for i in range(len(y)):
		...
		tmp2 = numpy.numpy.exp(c1)*numpy.exp(c2/(x[i] + c3)**2)
...
def adaptive_mask(vol, nsigma = 1.0, threshold = -9999.0, ndilation = 3, edge_width = 5, mode = "C"):
	...
	ny = vol.get_ysize()
	nz = vol.get_zsize()
	mc = utilities.utilities.model_circle(nx//2, nx, ny, nz) - utilities.utilities.model_circle(nx//3, nx, ny, nz)
...
def adaptive_mask2D(img, nsigma = 1.0, ndilation = 3, kernel_size = 11, gauss_standard_dev =9):
	...
	nx = img.get_xsize()
	ny = img.get_ysize()
	mc = utilities.utilities.model_circle(nx//2, nx, ny) - utilities.utilities.model_circle(nx//3, nx, ny)
...
#				write_text_file([freq, subpw.tolist(), ctf2, envelope.tolist(), baseline.tolist()], "%s/ravg%05d.txt" % (output_directory, ifi))
			...
			mask = utilities.utilities.model_circle(istop - 1, wn, wn) * (utilities.model_blank(wn, wn, 1, 1.0) - utilities.utilities.model_circle(istart, wn, wn))
		...
		if(len(adefocus) < 2):
			...
			if len(reject_img_messages) > 0:
				...
				for it in range(niter):
					EMAN2_cppwrap.Util.add_img(supe, EMAN2_cppwrap.EMAN2_cppwrap.Util.ctf_rimg(wn, wn, 1, ad1 + rqt.gauss(0.0,stdavad1), pixel_size, voltage, Cs, 0.0, wgh, bd1 + rqt.gauss(0.0,stdavbd1), cd1 + rqt.gauss(0.0,cd2), 1))
				ni = wn // 2
				supe /= niter
...
def angle2ampcont(phi):
	...
	return numpy.numpy.tan(numpy.numpy.radians(phi))/numpy.sqrt(1.0+numpy.tan(numpy.numpy.radians(phi))**2)*100.0
	...
	for ifi in range(len(ll)):
		...
		if True:  #try:
			for nboot in range(kboot):
				...
				mask = utilities.utilities.model_circle(istop-1,nx,nx)*(utilities.model_blank(nx,nx,1,1.0)-utilities.model_circle(istart,nx,nx))
				qse = qs #* envl
			...
			if(len(adefocus)<2):
				...
				cd2/=2
				temp = 0.0
				print(namics,ad1, Cs, voltage, Pixel_size, temp, wgh, bd1, cd1, numpy.numpy.sqrt(max(0.0,ad2)),numpy.sqrt(max(0.0,bd2)),cd2) 
				totresi.append( [ namics, ad1, Cs, voltage, Pixel_size, temp, wgh, bd1, cd1, numpy.numpy.sqrt(max(0.0,ad2)),numpy.sqrt(max(0.0,bd2)),cd2 ])
				#if ifi == 4 : break
				"""
				for i in xrange(len(ssubroo)):
					asubroo[i] /= kboot
	...
	for ifi in range(set_start, set_end):
		...
		if(len(adefocus) < 2):
			...
			if len(reject_img_messages) > 0:
				...
				niter = 1000
				for it in range(niter):
					EMAN2_cppwrap.Util.add_img(supe, EMAN2_cppwrap.EMAN2_cppwrap.Util.ctf_rimg(wn, wn, 1, ad1 + rqt.gauss(0.0,stdavad1), pixel_size, voltage, Cs, 0.0, ed1 + angle2ampcont(rqt.gauss(0.0,ed2)), bd1 + rqt.gauss(0.0,stdavbd1), cd1 + rqt.gauss(0.0,cd2), 1))
				ni = wn // 2
				supe /= niter
				pwrot2 = rotavg_ctf(supe, ad1, Cs, voltage, pixel_size, bd1, cd1)
				for i in range(ni):  pwrot2[i] = pwrot2[i]**2
...
def Xdefocusgett_vpp2(qse, roo, nx, xdefc, xampcont, voltage=300.0, Pixel_size=1.0, Cs=2.0, f_start=-1.0, f_stop=-1.0, round_off=1.0, nr1=3, nr2=6, parent=None, DEBug=False):
	...
	pass#IMPORTIMPORTIMPORT from alignment import Numrinit, ringwe
	mask = utilities.utilities.model_circle(i_stop - 1, wn, wn) * (utilities.model_blank(wn, wn, 1, 1.0) - utilities.utilities.model_circle(i_start, wn, wn))
	...
	crefim = EMAN2_cppwrap.Util.Polar2Dm(qse*mask, cnx, cny, numr, mode)
	print("  CREFIM    ",EMAN2_cppwrap.EMAN2_cppwrap.Util.infomask(qse*mask,None,True),EMAN2_cppwrap.EMAN2_cppwrap.Util.infomask(crefim,None,True))
	EMAN2_cppwrap.Util.Frngs(crefim, numr)
	EMAN2_cppwrap.Util.Applyws(crefim, numr, wr)
	bdef = 0.
