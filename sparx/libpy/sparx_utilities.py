...
def image_decimate(img, decimation=2, fit_to_fft=1,frequency_low=0, frequency_high=0):
	...
	if decimation    <= 1   :  	sparx_global_def.ERROR("Improper decimation ratio", "image_decimation", 1)
	if frequency_low <= 0   :
		frequency_low  = .5/decimation- .05
		if frequency_low <= 0: sparx_global_def.ERROR("Butterworth passband frequency is too low", "image_decimation", 1)
		frequency_high = .5/decimation+ .05
	if fit_to_fft :
		nx_d = (img.get_xsize())/int(decimation)
		ny_d = (img.get_ysize())/int(decimation)
		nx_fft_d = sparx_fundamentals.smallprime(int(nx_d))
		ny_fft_d = sparx_fundamentals.smallprime(int(ny_d))
		nx_fft_m = nx_fft_d*int(decimation)
		ny_fft_m = ny_fft_d*int(decimation)
		e   = sparx_fundamentals.window2d(img, nx_fft_m, ny_fft_m, "l")
		e1  = sparx_filter.filt_btwl(e, frequency_low, frequency_high)
		img = EMAN2_cppwrap.Util.decimate(e1, int(decimation), int(decimation), 1)
	else:

		e1  = sparx_filter.filt_btwl(img, frequency_low, frequency_high)
		img = EMAN2_cppwrap.Util.decimate(e1, int(decimation), int(decimation), 1)
	return  img
