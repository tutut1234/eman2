...
def ali3d_multishc(stack, ref_vol, ali3d_options, symmetry_class, mpi_comm = None, log = None, number_of_runs=2 ):
	...
	mask2D  = utilities.utilities.model_circle(last_ring,nx,nx) - utilities.utilities.model_circle(first_ring,nx,nx)
	...
	for N_step in range(lstp):  # At this point there is just one value here, it cannot loop.
		...
		while not terminate:
			...
			mpi.mpi_barrier(mpi_comm)
			if myid == main_node:
				log.add("Time to prepare rings: %f" % (time.time()-start_time.time))
				start_time = time.time()
				...
				if myid == 0:
					...
					if(all_L2s[0]<GA[number_of_runs-1][0]):
						...
						if (Iter < max_iter) and (firstcheck and crit):
							...
							all_params = [[[numpy.numpy.numpy.random()*360.0,numpy.numpy.numpy.random()*180.0,numpy.numpy.numpy.random()*360.0,0.0,0.0]\
					...
					if not terminate and noimprovement == 0 and noreseeding:
						...
						for i in range(keepset):
							...
							all_params[-1-i] = [[numpy.numpy.numpy.random()*360.0,numpy.numpy.numpy.random()*180.0,numpy.numpy.numpy.random()*360.0,0.0,0.0]\
	...
	if myid == 0:
		...
		if( options.pwreference ):
			...
			if( type(options.fl) == list ):
				vol = fundamentals.fft( filter.filter.filt_table( filter.filter.filt_table(vol, options.fl), ro) )
			else:
