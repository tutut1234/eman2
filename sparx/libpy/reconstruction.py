	...
	for im in range(len(prjlist)):
		...
		for ii in range(len(tdir)):
			...
			for ki in range(1,len(lshifts)):
				EMAN2_cppwrap.Util.add_img(recdata, EMAN2_cppwrap.EMAN2_cppwrap.Util.mult_scalar(prjlist[im][allshifts[lshifts[ki]]], probs[lshifts[ki]]/toprab))
...
def bootstrap_nn(proj_stack, volume_stack, list_proj, niter, media="memory", npad=4, symmetry="c1", output=-1, CTF=False, snr=1.0, sign=1, myseed=None ):
	...
	for i in range(niter):
		...
		if not(output is None):
			output.write( " done!" )
			output.write( " time.time.time %15.3f %15.3f \n" % (time()-iter_start,time()-overall_start) )
			output.flush()
	...
	for proj in projections:
		...
		for j in range(RA.get_nsym(symmetry)):
			...
			if use_weights:
				...
				EMAN2_cppwrap.Util.mul_scalar( proj3Dsphere, 1.0 / EMAN2_cppwrap.EMAN2_cppwrap.Util.infomask(proj3Dsphere, None, True)[3] )
...
def get_image_size( imgdata, myid ):
	...
	nimgs = mpi.mpi_gather( nimg, 1, mpi.mpi.MPI_INT, 1, mpi.mpi.MPI_INT, 0, mpi.MPI_COMM_WORLD )
