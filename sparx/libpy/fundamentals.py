...
def resample(img, sub_rate=0.5):
    ...
    if cc:
        ...
        utilities.set_params_proj(e, [cp["phi"], cp["theta"], cp["psi"], -cp["tx"],
...
def rot_shift2D(img, angle=0.0, sx=0.0, sy=0.0, mirror=0, scale=1.0, interpolation_method=None, mode="background"):
    ...
    if (interpolation_method):
        ...
        use_method = global_def.interpolation_method_2D
    ...
    if (use_method == "linear" and mode == "cyclic"):
        ...
        global_def.ERROR("rot_shift_2D interpolation method is incorrectly set", "rot_shift_2D", 1)
...
def window2d(img, isize_x, isize_y, opt="c", ix=0, iy=0):
    ...
    if (opt == "l"):
        ...
        global_def.ERROR("Unknown window2d option", "window2d", 1)
...
class symclass(object):
    ...
    #             nsym = 1
    #             redang = numpy.array([angles])
    #         else :
    #             nsym = self.nsym
    #             redang = numpy.array([angles]).repeat(nsym, axis=0)
    #
    #     elif self.sym[0] == "d":
    #         if angles[1] == 0 or angles[1] == 180:
    #             nsym = 2
    ...
    #             if angles[1] != 90 :
    #                 redang = numpy.array([angles]).repeat(self.nsym, axis=0)
    #             else:
    #                 redang = numpy.array([angles]).repeat(nsm, axis=0)
    #     else:
    #         redang = numpy.array([angles]).repeat(self.nsym, axis=0)
    #
    #
    ...
    #             # redang = numpy.array([angles]).repeat(self.nsym, axis=0)
    #             redang[:, 0] =  (redang[:, 0] + numpy.arange(redang.shape[0]) * qt) % 360.0
    #         else:
    #             print(numpy.shape(redang))
    #             redang = redang
    #
    #
    #     elif (self.sym[0] == "d"):
    ...
    #         if angles[1] == 0 or angles[1] ==180:
    #             # redang[1,:] = (0, 180, (redang[1,2] + 180.0 * (nsm % 2)) % 360.0)
    #             redang[:, 1] = (redang[:, 1] + 180 * numpy.arange(2)) % 360
    #             redang[:, 2] = (redang[:, 2] + 180 * numpy.arange(2)) % 360
    #
    #         else:
    #             redang[1:nsm, 0] =  ( redang[1:nsm,0] + numpy.arange(1,nsm) * qt) % 360.0
    #             redang[1:nsm, 1] = redang[1:nsm,1]
    #             redang[1:nsm, 2] = redang[1:nsm,2]
    #             if angles[1] != 90:
    #                 redang[nsm:self.nsym, 0]  = ( 360.0 - redang[numpy.arange(nsm,self.nsym) - nsm][:,0] ) % 360.0
    #                 redang[nsm:self.nsym, 1] =  180 -  redang[nsm:self.nsym, 1]
    #                 redang[nsm:self.nsym, 2] =  (redang[nsm:self.nsym, 2] + 180.0 * (nsm % 2)) % 360.0
    #     else:
    #         pass  # IMPORTIMPORTIMPORT from fundamentals import rotmatrix, recmat, mulmat
    #         mat = rotmatrix(angles[0], angles[1], angles[2])
    #         p1,p2,p3 = recmat( recmat(mulmat(mat, self.symatrix[numpy.arange(redang.shape[0])])))
    #         redang = ([p1,p2,p3])
    #
    #     return redang.tolist()
    def symmetry_related(self, angles):
        """
        Generate all symmetry related instances of input angles
        Input:  is a list of list of n triplets
    ...
    def symmetry_neighbors(self, angles):
        ...
        for i, q in enumerate(angles):
            ...
            for j, l in enumerate(neighbors[self.sym]):
                p1, p2, p3 = recmat(mulmat(mat, self.symatrix[l]))
                sang[i * (len(neighbors[self.sym]) + 1) + (j + 1)] = [p1, p2, p3]
        return sang
