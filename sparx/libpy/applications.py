...
#  I did not check the version with no check mirror, I doubt it works.
	...
	for im in range(nima):
		if randomize:
			...
			if check_mirror: alphan, sxn, syn, mirrorn = utilities.combine_params2(0.0, -sxi, -syi, 0, numpy.random()*360.0, 0.0, 0.0, random.randint(0, 1))
			else:            alphan, sxn, syn, mirrorn = utilities.combine_params2(0.0, -sxi, -syi, 0, numpy.random()*360.0, 0.0, 0.0, 0)			
...
def ali3d_MPI(stack, ref_vol, outdir, maskfile = None, ir = 1, ou = -1, rs = 1, 
	...
	for N_step in range(lstp):
		...
		while Iter < max_iter-1 and terminate == 0:
			...
			if myid == main_node:
				utilities.print_msg("3D reconstruction time.time = %d\n"%(time()-start_time))
				start_time = time.time()
...
def sali3d_base(stack, ref_vol = None, Tracker = None, rangle = 0.0, rshift = 0.0, mpi_comm = None, log = None):
	...
	# log
	if myid == main_node:
		log.add("Setting of reference 3D reconstruction time.time = %10.1f\n"%(time()-start_time))
		start_time = time.time()
	...
	for N_step in range(lstp):
		...
		while Iter < max_iter and terminate == 0:
			...
			# centering and volume reconstruction if not terminating
				...
				# log
				if myid == main_node:
					log.add("3D reconstruction time.time = %10.1f\n"%(time()-start_time))
					start_time = time.time()
			#=========================================================================
		...
		for ic in range(n_of_chunks):
			...
			if( ((iteration > 0) and (ic > 0)) or vol == None):
				...
				if myid == main_node:
					...
					log.add("3D reconstruction time.time = %d"%(time()-start_time))
					start_time = time.time()
				if debug:
...
def mref_ali3d_MPI(stack, ref_vol, outdir, maskfile=None, focus = None, maxit=1, ir=1, ou=-1, rs=1, \
	...
	for Iter in range(Niter):
		...
		for iref in range(numref):
			...
			if CTF:
				...
				if runtype=="REFINEMENT":
					...
					if(myid == 0):
						log.add( "Calculation of projections: %d" % (time()-start_time) );start_time = time.time()
					del volft, kb
				...
				if runtype=="REFINEMENT":
					...
					refrings = alignment.prepare_refrings( volft, kb, nx, delta[N_step], ref_a, sym, numr)
					if(myid == 0):
						log.add( "Initial time to prepare rings: %d" % (time()-start_time) );start_time = time.time()
					del volft, kb
				...
				if(CTF):
					...
					if runtype=="REFINEMENT":
						if(ctf.defocus != previous_defocus):
							...
							if(myid == 0):
								log.add( "Repeated time to prepare rings: %d" % (time()-rstart_time) );rstart_time = time.time()
		...
		for iref in range(numref):
			...
			if(myid == 0):
				log.add( "Time to compute 3D: %d" % (time()-start_time) );start_time = time.time()
		...
		if terminate ==1: # headers are only updated when the program is going to terminate
			...
			if(myid == 0):
				log.add( "Time to write headers: %d\n" % (time()-start_time) );start_time = time.time()
			mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
			if myid==main_node:
...
def Kmref2_ali3d_MPI(stack, ref_vol, outdir, maskfile=None, focus = None, maxit=1, ir=1, ou=-1, rs=1, 
	...
	for Iter in range(Niter):
		...
		for iref in range(numref):
			...
			del volft, kb
			if myid == main_node:
				log.add( "Time to prepare %d rings: %g" % (len(refrings),time()-start_time) );start_time = time.time()
...
def ihrsr_MPI(stack, ref_vol, outdir, maskfile, ir, ou, rs, xr, ynumber,\
	...
	symmetry_string = numpy.split(symmetryLower)[0]
	...
	for N_step in range(lstp):
		...
		while(Iter < max_iter and terminate == 0):
			...
			if(myid==main_node):
				...
				utilities.drop_image(vol, os.path.join(outdir, "volf%04d.hdf"%(total_iter)))
				utilities.print_msg("\nSymmetry search and user function time.time = %d\n"%(time()-start_time))
				start_time = time.time()
...
def ra_cef(indir, noise, outdir, prf, num):
	...
	for i, v in enumerate(flist):
		(filename,filextension) = os.os.path.splitext(v)
		if(filename[0:4] == "ptl_"):
			infile  = os.os.path.join(indir, v)
			outfile = os.os.path.join(outdir,prf+v)
			for j in range(num):
...
def recons3d_n_MPI(prj_stack, pid_list, vol_stack, CTF=False, snr=1.0, sign=1, npad=2, sym="c1", listfile="", group=-1, verbose=0, xysize=-1, zsize=-1, smearstep = 0.0):
	...
	if myid == 0 :
		...
		if not(finfo is None):
			...
			finfo.write( "Total time.time: %10.3f\n" % (time()-time_start) )
			finfo.flush()
		...
		if not given:
			...
			for n in range(len(Prj)):
				if first_zero and n == 0:
					...
					Ori[j]   = numpy.random() * 360  # phi
					Ori[j+1] = numpy.random() * 180  # theta
					Ori[j+2] = numpy.random() * 360  # psi
				j += 4
		...
		if fimport != None:
			...
			for p in params:
				...
				if zero:
					...
					if p[:13] == "xform.align2d":
						alpha = numpy.random()*360.0
						sx = numpy.random()*2.0-1.0
						sy = numpy.random()*2.0-1.0
						...
						phi = numpy.random()*360.0
						theta = numpy.random()*180.0
						psi = numpy.random()*360.0
						s2x = numpy.random()*4.0-2.0
						s2y = numpy.random()*4.0-2.0
						...
						theta = numpy.random()*180.0
						psi = numpy.random()*360.0
						s3x = numpy.random()*4.0-2.0
						s3y = numpy.random()*4.0-2.0
						s3z = numpy.random()*4.0-2.0
						mirror = random.randint(0, 1)
						scale = 1.0
...
def Xwithin_group_refinement(data, maskfile, randomize, ir, ou, rs, xrng, yrng, step, dst, maxit, FH, FF, method = ""):
	...
	for im in range(nima):
		...
		syi -= ly[im]
		if randomize :
			alphan, sxn, syn, mirrorn    = utilities.combine_params2(0.0, sxi, syi, 0, numpy.random()*360.0, 0.0, 0.0, random.randint(0, 1))
...
def localhelicon_MPInew(stack, ref_vol, outdir, seg_ny, maskfile, ir, ou, rs, xr, ynumber,\
	...
	ref_a           = "P"
	symmetry_string = numpy.split(sym)[0]
...
#  					setfilori_SP(data[seg_start: seg_end], pixel_size, dp, dphi)
			...
			if (Iter-1) % search_iter == 0 :
				...
				if myid == main_node:
					...
					vol = utilities.sym_vol(vol, symmetry=sym)
					utilities.drop_image(vol, os.path.join(outdir, "volf%04d.hdf"%(total_iter)))
					utilities.print_msg("Symmetry enforcement and user function time.time = %d\n"%(time()-start_time))
					start_time = time.time()
				...
				if CTF:
					...
					if runtype=="REFINEMENT":
						if ctf.defocus != previous_defocus:
							...
							rstart_time = time.time()
							refrings = projection.gen_rings_ctf( prjref, nx, ctf, numr)
							if myid == main_node: log.add( "Repeated time to prepare rings: %d" % (time()-rstart_time) );rstart_time = time.time()
				if runtype=="ASSIGNMENT":
					phi,tht,psi,s2x,s2y = utilities.get_params_proj(data[im])
					'''
					if CTF:
		...
		for iref in range(numref):
			...
			if CTF:
				...
				if runtype=="REFINEMENT":
					start_time = time.time()
					prjref = projection.prgq( volft, kb, nx, delta[N_step], ref_a, sym, MPI=True)
					if(myid == 0): log.add( "Calculation of projections: %d" % (time()-start_time) );start_time = time.time()
					del volft, kb
			else:
				if runtype=="REFINEMENT":
					start_time = time.time()
					refrings = alignment.prepare_refrings( volft, kb, nx, delta[N_step], ref_a, sym, numr)
					if(myid == 0): log.add( "Initial time to prepare rings: %d" % (time()-start_time) );start_time = time.time()
					del volft, kb
			start_time = time.time()
			for im in range(nima):
				if(CTF):
					...
					if runtype=="REFINEMENT":
						if(ctf.defocus != previous_defocus):
							...
							rstart_time = time.time()
							refrings = projection.gen_rings_ctf( prjref, nx, ctf, numr)
							if(myid == 0): log.add( "Repeated time to prepare rings: %d" % (time()-rstart_time) );rstart_time = time.time()
