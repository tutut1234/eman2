...
#  I did not check the version with no check mirror, I doubt it works.
	...
	for im in range(nima):
		if randomize:
			...
			if check_mirror: alphan, sxn, syn, mirrorn = utilities.combine_params2(0.0, -sxi, -syi, 0, random.random()*360.0, 0.0, 0.0, random.randint(0, 1))
			else:            alphan, sxn, syn, mirrorn = utilities.combine_params2(0.0, -sxi, -syi, 0, random.random()*360.0, 0.0, 0.0, 0)			
...
def Xwithin_group_refinement(data, maskfile, randomize, ir, ou, rs, xrng, yrng, step, dst, maxit, FH, FF, method = ""):
	...
	for im in range(nima):
		...
		syi -= ly[im]
		if randomize :
			alphan, sxn, syn, mirrorn    = utilities.combine_params2(0.0, sxi, syi, 0, random.random()*360.0, 0.0, 0.0, random.randint(0, 1))
