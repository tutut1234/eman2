...
def main():
	...
	if len(args) < 2 or len(args) > 3:
		...
		sparx_applications.ali2d_rotationaltop(args[1], args[0], options.randomize, options.orient, options.ir, options.ou, options.rs, options.psi_max, options.mode, options.maxit)
	else:
		...
		if  options.MPI:
			...
			if(myid == main_node):
				...
				outcome = 0
				log = None
			outcome  = sparx_utilities.bcast_number_to_all(outcome, source_node = main_node)
			if(outcome == 1):
				sparx_global_def.ERROR('Output directory exists, please change the name and restart the program', "ali2d_MPI", 1, myid)

			dummy = sparx_applications.ali2d_base(args[0], outdir, mask, options.ir, options.ou, options.rs, options.xr, options.yr, \
