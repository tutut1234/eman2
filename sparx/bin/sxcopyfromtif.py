...
def main():
	...
	if len(args) < 1:
		...
		sparx_applications.copyfromtif(args[0], outdir, options.inx, options.foc, options.ext, options.cst, options.pixel_size, options.sca_a, options.sca_b, options.step, options.mag, options.MPI)
