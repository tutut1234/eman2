...
def main():
	...
	if len(args) < 1  or len(args) > 4:
		...
		if(myid == main_node):
			...
			a = sparx_utilities.get_im(Tracker["orgstack"])
			nnxo = a.get_xsize()
...
####-----------------------------------------------------------------------------------------
		...
		if myid == main_node:
			...
			sparx_utilities.cmdexecute(cmd)
		...
		if myid ==main_node:
			if Tracker["constants"]["mask3D"]:
				sparx_morphology.get_shrink_3dmask(Tracker["nxinit"],Tracker["constants"]["mask3D"]).write_image(Tracker["mask3D"])
		...
		if Tracker["constants"]["PWadjustment"]:
			...
			if myid ==main_node:
				sparx_utilities.write_text_file(nxinit_pwsp,Tracker["nxinit_PW"])
		...
		if myid ==main_node:
			...
			stable_member_list                              = sparx_utilities.get_stable_members_from_two_runs(Tracker["constants"]["previous_runs"], Tracker["constants"]["total_stack"], log_main)
			Tracker["this_unaccounted_list"], new_stable_P1 = sparx_utilities.get_leftover_from_stable(stable_member_list, Tracker["constants"]["total_stack"], Tracker["constants"]["smallest_group"])
		...
		stable_member_list               = sparx_utilities.wrap_mpi_bcast(stable_member_list, main_node)
		Tracker["total_stack"]           = sparx_utilities.bcast_number_to_all(Tracker["total_stack"], source_node = main_node)
		left_one_from_old_two_runs       = sparx_utilities.wrap_mpi_bcast(Tracker["this_unaccounted_list"], main_node)
		if myid ==main_node:  
			sparx_utilities.write_text_file(left_one_from_old_two_runs, os.path.join(masterdir,"unaccounted_from_two_previous_runs.txt"))
		...
		if Tracker["constants"]["chunkdir"] !="": ##inhere previous random assignment of odd and even
			if myid == main_node:
				chunk_one = sparx_utilities.read_text_file(os.path.join(Tracker["constants"]["chunkdir"],"chunk0.txt"))
				chunk_two = sparx_utilities.read_text_file(os.path.join(Tracker["constants"]["chunkdir"],"chunk1.txt"))
			else:
				chunk_one = 0
				chunk_two = 0
		...
		if myid == main_node:
			sparx_utilities.write_text_file(chunk_one, os.path.join(masterdir,"chunk0.txt"))
			sparx_utilities.write_text_file(chunk_two, os.path.join(masterdir,"chunk1.txt"))
		mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
		vols = []
		for index in range(2):
			data1,old_shifts1 = sparx_utilities.get_shrink_data_huang(Tracker,Tracker["constants"]["nxinit"], os.path.join(masterdir,"chunk%d.txt"%index), Tracker["constants"]["partstack"], myid, main_node, nproc, preshift = True)
			vol1 = sparx_reconstruction.recons3d_4nn_ctf_MPI(myid=myid, prjlist=data1, symmetry=Tracker["constants"]["sym"], finfo=None)
			if myid ==main_node:
			...
			mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
		if myid ==main_node:
			low_pass, falloff, currentres = sparx_utilities.get_resolution_mrk01(vols, Tracker["constants"]["radius"]*Tracker["shrinkage"], Tracker["constants"]["nxinit"], masterdir,Tracker["mask3D"])
			if low_pass   > Tracker["constants"]["low_pass_filter"]: low_pass  = Tracker["constants"]["low_pass_filter"]
		else:
			low_pass    =0.0
			falloff     =0.0
			currentres  =0.0
		currentres                    = sparx_utilities.bcast_number_to_all(currentres,source_node = main_node)
		low_pass                      = sparx_utilities.bcast_number_to_all(low_pass,source_node   = main_node)
		falloff                       = sparx_utilities.bcast_number_to_all(falloff,source_node    = main_node)
		...
		if myid ==main_node:
			...
			sparx_filter.filt_tanl(sparx_utilities.get_im(os.path.join(masterdir, "vol0.hdf")), Tracker["low_pass_filter"], 0.1).write_image(os.path.join(masterdir, "volf0.hdf"))			
			sparx_filter.filt_tanl(sparx_utilities.get_im(os.path.join(masterdir, "vol1.hdf")), Tracker["low_pass_filter"], 0.1).write_image(os.path.join(masterdir, "volf1.hdf"))
			print(" random odd and even assignment done  !")
		mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
		## ---------------------------------------------------------------------------------------------########
		## Stop program and output results when the leftover from two sort3d runs is not sufficient for a new run    		########
		## ---------------------------------------------------  ---------------------------------------  ######
		Tracker["number_of_groups"] = sparx_utilities.get_number_of_groups(len(left_one_from_old_two_runs), Tracker["constants"]["number_of_images_per_group"])
		if Tracker["number_of_groups"] <=1 : # programs finishes 
			if myid == main_node:
				...
				for i in range(len(stable_member_list)): sparx_utilities.write_text_file(stable_member_list[i], os.path.join(masterdir,"P2_final_class%d.txt"%i))
				mask3d = sparx_utilities.get_im(Tracker["constants"]["mask3D"])
			else:
				mask3d = sparx_utilities.model_blank(Tracker["constants"]["nnxo"],Tracker["constants"]["nnxo"],Tracker["constants"]["nnxo"])
			...
			for igrp in range(len(stable_member_list)):
				...
				data, old_shifts = sparx_utilities.get_shrink_data_huang(Tracker,Tracker["constants"]["nnxo"], os.path.join(masterdir, "P2_final_class%d.txt"%igrp), Tracker["constants"]["partstack"], myid, main_node, nproc,preshift = True)
				if Tracker["constants"]["CTF"]:  
					volref, fscc = sparx_reconstruction.rec3D_two_chunks_MPI(data, 1.0, Tracker["constants"]["sym"], mask3d,os.path.join(masterdir,"resolution_%02d.txt"%igrp), myid, main_node, index =-1, npad=2)
				else: 
				...
				if myid ==main_node:
					try: 
						lowpass = sparx_utilities.search_lowpass(fscc)
						falloff = 0.1
					except:
						lowpass = 0.4
						falloff = 0.1
			...
			for iter_P2_run in range(number_of_P2_runs): # two runs such that one can obtain reproducibility
				...
				Tracker["number_of_groups"] = sparx_utilities.get_number_of_groups(total_stack, Tracker["constants"]["number_of_images_per_group"])
				if myid == main_node:
				...
				while( Tracker["number_of_groups"]>=2 ):
					...
					###------ For super computer    ##############
					sparx_utilities.update_full_dict(list_to_be_processed, Tracker)
					###----
					...
					for indep_run in range(Tracker["constants"]["indep_runs"]):
						...
						if myid ==main_node:
							sparx_utilities.write_text_file(list_to_be_processed, os.path.join(workdir, "independent_list_%03d.txt"%indep_run))
						sparx_applications.mref_ali3d_EQ_Kmeans(ref_vol, os.path.join(workdir, "EQ_Kmeans%03d"%indep_run), os.path.join(workdir, "independent_list_%03d.txt"%indep_run), Tracker)
					...
					if myid ==main_node:
						for igrp in range(len(Tracker["two_way_stable_member"])):
							...
							sparx_utilities.write_text_file(Tracker["this_data_list"], os.path.join(workdir,"stable_class%d.txt"%igrp))
					...
					ref_vol_list = []
					for igrp in range(len(Tracker["two_way_stable_member"])):
						data, old_shifts = sparx_utilities.get_shrink_data_huang(Tracker,Tracker["nxinit"], os.path.join(workdir, "stable_class%d.txt"%igrp), Tracker["constants"]["partstack"], myid, main_node, nproc, preshift = True)
					...
					empty_groups,res_classes,final_list  = sparx_applications.ali3d_mref_Kmeans_MPI(ref_vol_list, outdir, os.path.join(workdir,"Accounted.txt"), Tracker)
					...
					if myid == main_node: sparx_utilities.write_text_file(Tracker["this_unaccounted_list"], Tracker["this_unaccounted_text"])
					...
					if myid == main_node:
						...
						for igrp in range(Tracker["number_of_groups"]):
							if os.path.exists( os.path.join( outdir,"Class%d.txt"%igrp ) ):
								new_stable1.append( sparx_utilities.read_text_file( os.path.join( outdir, "Class%d.txt"%igrp ) ) )
								log_main.add(" read Class file %d"%igrp)
								number_of_ref_class.append(len(new_stable1))
					else:  number_of_ref_class = 0
					number_of_ref_class = sparx_utilities.wrap_mpi_bcast(number_of_ref_class,main_node)
					...
					if myid ==main_node:
						...
						sparx_filter.filt_tanl(vol_list[ivol],Tracker["constants"]["low_pass_filter"],.1).write_image(os.path.join(workdir, "volf_of_Classes.hdf"),ivol)
					...
					Tracker["number_of_groups"]      = sparx_utilities.get_number_of_groups(len(list_to_be_processed),Tracker["constants"]["number_of_images_per_group"])
					mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
	...
	#############################################################################################################################
				...
				if (Tracker["constants"]["unaccounted"] and (len(Tracker["this_unaccounted_list"]) != 0)):
					...
					volref = sparx_reconstruction.recons3d_4nn_ctf_MPI(myid=myid, prjlist = data, symmetry=Tracker["constants"]["sym"],finfo=None)
					volref = sparx_filter.filt_tanl(volref, Tracker["constants"]["low_pass_filter"],.1)
				...
				if myid ==main_node:
					...
					unaccounted                 = sparx_utilities.get_complementary_elements_total(Tracker["constants"]["total_stack"], final_list)
				...
				Tracker["number_of_groups"] = sparx_utilities.bcast_number_to_all(Tracker["number_of_groups"],source_node = main_node)
				mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
				final_list = sparx_utilities.wrap_mpi_bcast(final_list, main_node)
				workdir = os.path.join(P2_run_dir,"Exhaustive_Kmeans") # new workdir 
				if myid==main_node:
					os.mkdir(workdir)
					sparx_utilities.write_text_file(final_list, os.path.join(workdir,"final_list.txt"))
				else: new_stable1 = 0
				mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
				...
				if myid == main_node:
					...
					for igrp in range(Tracker["number_of_groups"]):
						...
						sparx_utilities.write_text_file(new_stable1[igrp],class_file)
				...
				for igrp in range(Tracker["number_of_groups"]):
					...
					data,old_shifts                = sparx_utilities.get_shrink_data_huang(Tracker, Tracker["nxinit"],os.path.join(workdir,"final_class%d.txt"%igrp), Tracker["constants"]["partstack"], myid,main_node,nproc,preshift = True)
					volref                         = sparx_reconstruction.recons3d_4nn_ctf_MPI(myid=myid, prjlist = data, symmetry=Tracker["constants"]["sym"], finfo = None)
					...
					sparx_utilities.bcast_EMData_to_all(volref, myid, main_node)
					ref_vol_list.append(volref)
				mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
				...
				KE_group                       = sparx_utilities.Kmeans_exhaustive_run(ref_vol_list,Tracker) # 
			...
			reproduced_groups = sparx_utilities.two_way_comparison_single(P2_partitions[0],P2_partitions[1],Tracker)# Here partition IDs are original indexes.
			...
			if myid == main_node:
				for index_of_reproduced_groups in range(len(reproduced_groups)):
					...
					sparx_utilities.write_text_file(reproduced_groups[index_of_reproduced_groups],name_of_class_file)
				log_main.add("-------start to reconstruct reproduced volumes individully to orignal size-----------")
				
			mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
			if Tracker["constants"]["mask3D"]: mask_3d = sparx_morphology.get_shrink_3dmask(Tracker["constants"]["nnxo"],Tracker["constants"]["mask3D"])
			else:                              mask_3d = None
			...
			for igrp in range(len(reproduced_groups)):
				...
				if Tracker["constants"]["CTF"]: 
					volref, fscc = sparx_reconstruction.rec3D_two_chunks_MPI(data,1.0,Tracker["constants"]["sym"],mask_3d, \
					...
					exit()
				mpi.mpi_barrier(mpi.MPI_COMM_WORLD)
				fscc        = sparx_utilities.read_text_file(os.path.join(masterdir, "resolution_%02d.txt"%igrp),-1)
				nx_of_image = volref.get_xsize()
