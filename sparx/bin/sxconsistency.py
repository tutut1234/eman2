...
def errors_per_image_original(nn, qt, asi, avgtrans, thresherr=1.0, radius = 1.0):
	...
	for k in range(nn):
		...
		assert(len(fifi) == 3)
		nas = [0.0,0.0,0.0]
		r1,r2,r3 = sparx_utilities.getfvec(avgtrans[k][0],avgtrans[k][1])
		sacos = 0.0
		sder = 0.0
		ser3 = 0.0
		for i in range(3):
			#print i,fifi[i]
			d = sparx_pixel_error.max_3D_pixel_error(fifi[i], avgtrans[k], r=radius)
			#if(d>10.):  print  "  LARGE ERROR",k,i,d,fifi[i], avgtrans[k]
			ser3 += d
			n1,n2,n3 = sparx_utilities.getfvec(fifi[i][0],fifi[i][1])
			sacos += sparx_utilities.lacos(n1*r1+n2*r2+n3*r3)
			sder += sparx_pixel_error.pixel_error_2D(fifi[i][2:], avgtrans[k][2:], r = 1.0)
...
def errors_per_image(params, avgtrans, thresherr=1.0, radius = 1.0):
	...
	for k in range(nn):
		...
		for i in range(ll):
			#print i,fifi[i]
			d = sparx_pixel_error.max_3D_pixel_error(params[i][k], avgtrans[k], r=radius)
			#if(d>10.):  print  "  LARGE ERROR",k,i,d,fifi[i], avgtrans[k]
			ser3 += d
			n1,n2,n3 = sparx_utilities.getfvec(params[i][k][0],params[i][k][1])
			sacos += sparx_utilities.lacos(n1*r1+n2*r2+n3*r3)
			sder += sparx_pixel_error.pixel_error_2D(params[i][k][2:], avgtrans[k][2:], r = 1.0)
		# average deviation in radians
		sacos /= ll
		sare   = numpy.tan(sacos)
		sder  /= ll
...
def average_trans(params):
	...
	for j in range(nn):
		...
		if( sym == 1):
			...
			for i in range(ll):
				n1,n2,n3 = sparx_utilities.getfvec(params[i][j][0],params[i][j][1])
				nas[0] += n1
				nas[1] += n2
				nas[2] += n3
		else:
			m1,m2,m3 = sparx_utilities.getfvec(params[0][j][0],params[0][j][1])
			nas[0] = m1
			nas[1] = m2
			nas[2] = m3
			for i in range(1,ll):
				...
				for j in range(-1,2,1):
					t1,t2,t3 = sparx_utilities.getfvec(params[i][j][0]+j*qsym,params[i][j][1])
...
def main():
	...
	if options.phase == 1 and len(args) == 2:
		...
		for i in range(4):
			...
			j,k = sparx_applications.MPI_start_end(nn, 4, i)
			chunks.append(t[j:k])
			chunks[i].sort()
			sparx_utilities.write_text_file(chunks[i],os.path.join(outdir,'chunk%01d.txt'%i))

		del t
		sparx_utilities.write_text_file([ len(chunks[i]) for i in range(4) ],os.path.join(outdir,'chunklengths.txt'))
		...
		prms = []
		for i in range(6):
			prms.append( sparx_utilities.read_text_row(os.path.join(outdir,options.params+"%01d.txt"%i)) )
		...
		sparx_utilities.write_text_file([qt], os.path.join(outdir,howmanythesame) )
	...
	#########     PHASE 3
		...
		chunklengths = {}
		for i in range(4):
			chunks[chr(65+i)] = list(map(int,sparx_utilities.read_text_file(os.path.join(outdir,options.chunk+"%01d.txt"%i))))
			chunklengths[chr(65+i)] = len(chunks[chr(65+i)])


		for i in range(6):
			prms = sparx_utilities.read_text_row(os.path.join(outdir,options.params+"%01d.txt"%i))
			for q in blocks:
		...
		for i,q in enumerate(pairs):
			...
			for j in range(chunklengths[q[0][0]]):
				...
				nas = [0.0,0.0,0.0]
				if( sym == 1):
					pixer[q[0][0]][j] = sparx_pixel_error.max_3D_pixel_error(fifi[0], fifi[1], r=radius)
					for i in range(2):
						n1,n2,n3 = sparx_utilities.getfvec(fifi[i][0],fifi[i][1])
						nas[0] += n1
						nas[1] += n2
					...
					m1,m2,m3 = sparx_utilities.getfvec(fifi[0][0],fifi[0][1])
					nas[0] = m1
					nas[1] = m2
					nas[2] = m3
					#if(k == 2):
					...
					for i in range(1,2):
						qnom = -1.e10
						for j in range(-1,2,1):
							t1,t2,t3 = sparx_utilities.getfvec(fifi[i][0]+j*qsym,fifi[i][1])
							nom = t1*m1 + t2*m2 + t3*m3
							if(nom > qnom):
								qnom = nom
								n1=t1
		...
		for q in blocks:
			...
			if(tgood < 4):
				...
				exit()
		print(" tgood ", tgood)
		hi = sparx_statistics.hist_list([pixer[q][k] for q in blocks for k in range(chunklengths[q])  ],16)
		...
		for i,q in enumerate(blocks):
			for k in range(chunklengths[q]):
				...
				if  perr[q][k]:
					...
					#[chunks[q][k],pixer[q][k],deprt, params[pairs[i][0]][k],params[pairs[i][1]][k],params[lefts[i]][k],avgtrans[q][k]])
			sparx_utilities.write_text_file( good[i], os.path.join(outdir,"newgood%01d.txt"%i) )
			sparx_utilities.write_text_file( bad[i],  os.path.join(outdir,"newbad%01d.txt"%i) )
			...
			for m in range(6):
				if q+chr(48+m) in params:
					...
					sparx_utilities.write_text_row(prmsgood, os.path.join(outdir,"params-newgood%01d%01d.txt"%(i,ll)))
					sparx_utilities.write_text_row(prmsbad, os.path.join(outdir, "params-newbad%01d%01d.txt"%(i,ll)))
					ll += 1
		...
		for i in range(3):
			for j in range(i+1,4):
				sparx_utilities.write_text_file( good[i] + good[j], os.path.join(outdir,"newlili%01d.txt"%ll) )
				ll += 1
			...
			for q in blocks:
				if q+chr(48+i) in params:
					try:
						...
						prms += avgtrans[q]
			sparx_utilities.write_text_row(prms, os.path.join(outdir,outgrouparms+"%01d.txt"%i))


		#  Write chunklengths
		chunklengths = [len(good[i]) for i in range(4)]
		sparx_utilities.write_text_file(chunklengths, os.path.join(outdir,"chunklengths.txt") )
		"""
		...
		for jj in range(4):
			...
			newbad = list(map(int, sparx_utilities.read_text_file(options.params+"%01d.txt"%jj) ))
			nn = len(newbad)
			for ii in range(ll):
				params[ii] = sparx_utilities.read_text_row(os.path.join(outdir,bp+"%01d%01d.txt"%(jj,ii)))
			...
			if( len(rescued) == 0 ):    sparx_utilities.write_text_row([-1,-1],  os.path.join(outdir,"rescued%01d.txt"%jj))
			else:                  		sparx_utilities.write_text_row(rescued,  os.path.join(outdir,"rescued%01d.txt"%jj))
			#  We also have to write params.
			if( len(rescued) != 0 ):
				for ii in range(ll):  sparx_utilities.write_text_row([params[ii][rescued[k][1]] for k in range(len(rescued))]  ,  os.path.join(outdir,"params-rescued%01d%01d.txt"%(jj,ii)))
			if( len(rejects) == 0 ):    sparx_utilities.write_text_row([-1,-1],  os.path.join(outdir,'rejects'+"%01d.txt"%jj))
			else:                  		sparx_utilities.write_text_row(rejects,  os.path.join(outdir,'rejects'+"%01d.txt"%jj))
			if( len(rejects) != 0 ):
				for ii in range(ll):  sparx_utilities.write_text_row([params[ii][rejects[k][1]] for k in range(len(rejects))]  ,  os.path.join(outdir,"params-rejects%01d%01d.txt"%(jj,ii)))

			hi = sparx_statistics.hist_list([perr[j][0]  for j in range(nn)  ], 16)
		...
		for jj in range(1):
			...
			#nn = len(newbad)
			for ii in range(ll):
				params[ii] = sparx_utilities.read_text_row(os.path.join(outdir,"params%01d.txt"%(ii)))
				#assert(nn == len(params[ii]) )
			nn = len(params[0])
			...
			sparx_utilities.write_text_row(avgtrans,  os.path.join(outdir,"avgtrans.txt"))
			perr =  errors_per_image(params, avgtrans, thresherr, radius )
			rescued = []
			rejects = []
			for j in range(nn):
