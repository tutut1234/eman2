...
def get_shrink_data_final(nxinit, procid, original_data = None, oldparams = None, \
	...
	for im in range(nima):
		...
		sparx_utilities.set_params_proj(data[im],[phi,theta,psi,max(min(sx*shrinkage,txm),txl),\
...
def do_boxes_two_way_comparison_mpi(nbox, input_box_parti1,\
	...
	if nclass == 0:
		...
		if Blockdata["myid"]==Blockdata["main_node"]:
			...
			else:               unaccounted_list = copy.copy.copy(utmp)
...
def do3d(procid, data, newparams, refang, rshifts, norm_per_particle, myid, mpi_comm = -1):
	...
	tvol, tweight, trol = sparx_reconstruction.recons3d_trl_struct_MPI(myid = \
...
def do3d_sorting_groups_rec3d(iteration, masterdir, log_main):
	...
	if (Blockdata["no_of_groups"]>1): # multiple nodes
		...
		for iloop in range(nbig_loop):
			for im in range(len(big_loop_colors[iloop])):
				...
				if(Blockdata["myid"] == Blockdata["last_node"]):
					tvol2 		= sparx_utilities.get_im(os.path.join(Clusterdir, "tempdir", "tvol_0_%03d.hdf"%iteration))
					tweight2 	= sparx_utilities.get_im(os.path.join(Clusterdir, "tempdir", "tweight_0_%03d.hdf"%iteration))
					treg2 		= sparx_utilities.get_im(os.path.join(Clusterdir, "tempdir", "trol_0_%03d.hdf"%iteration))
					tag      = 7007
					sparx_utilities.send_EMData(tvol2,    sub_main_node_list[index_of_colors],  tag, mpi.MPI_COMM_WORLD)
					sparx_utilities.send_EMData(tweight2, sub_main_node_list[index_of_colors],  tag, mpi.MPI_COMM_WORLD)
					sparx_utilities.send_EMData(treg2,    sub_main_node_list[index_of_colors],  tag, mpi.MPI_COMM_WORLD)
			...
			for im in range(len(big_loop_colors[iloop])):
				...
				Clusterdir = os.path.join(Tracker["directory"], "Cluster%d"%index_of_group, "main%03d"%iteration)
				if(Blockdata["myid"] == Blockdata["last_node"]):
					tvol2 		= sparx_utilities.get_im(os.path.join(Clusterdir, "tempdir", "tvol_1_%03d.hdf"%iteration))
					tweight2 	= sparx_utilities.get_im(os.path.join(Clusterdir, "tempdir", "tweight_1_%03d.hdf"%iteration))
					treg2 		= sparx_utilities.get_im(os.path.join(Clusterdir, "tempdir", "trol_1_%03d.hdf"%iteration))
					tag      = 7007
					sparx_utilities.send_EMData(tvol2,    sub_main_node_list[index_of_colors], tag,mpi.MPI_COMM_WORLD)
					sparx_utilities.send_EMData(tweight2, sub_main_node_list[index_of_colors], tag,mpi.MPI_COMM_WORLD)
					sparx_utilities.send_EMData(treg2,    sub_main_node_list[index_of_colors], tag,mpi.MPI_COMM_WORLD)
	...
	Tracker = sparx_utilities.wrap_mpi_bcast(Tracker, Blockdata["main_node"])
	if keepgoing == 0: 
		sparx_global_def.ERROR("do3d_sorting_groups_trl_iter  %s"%os.path.join(Tracker["directory"], \
		   "tempdir"),"do3d_sorting_groups_trl_iter", 1, Blockdata["myid"]) 
	return
####=====<-----------------------------------------------------------------------------
### nofsc rec3d
...
def AI_freeze_groups(min_size, groups):
	...
	if number_of_groups>1:
		...
		return sparx_statistics.table_stat(Kmeans_size), Kmeans_size
	else:
