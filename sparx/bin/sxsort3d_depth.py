...
def estimate_tanhl_params(cutoff, taa, image_size):
	...
	def tanhfl(x, cutoff, taa):
		...
		v1    = (cnst*(x + omega))
		v2    = (cnst*(x - omega))
		return 0.5*(numpy.tanh(v1) - numpy.numpy.tanh(v2))
	...
	if search_mode: # Search AI
		if len(clusters)>2:
			if sum(glist) == len(glist)-1:cutoff = coid[1]
				...
				if ratio >33.:
					if numpy.numpy.sqrt(sstat[1])/sstat[0] > 0.50: cutoff   = int(coid[0] + numpy.numpy.sqrt(sstat[1]))  #int(2*current_img_per_grp)
...
def downsize_data_for_sorting(original_data, return_real = False, preshift = True, npad = 1, norms = None):
	...
	for im in range(nima):
		...
		if Tracker["constants"]["focus3D"]:
			...
			cdata[im] = fundamentals.fundamentals.fft(focusmask*fundamentals.fft(cdata[im]))
			fdata[im] = focusmask
...
def compare_two_images_eucd(data, ref_vol, fdata, ctfimgs):
	...
	for im in range(len(data)):
		...
		if Tracker["constants"]["focus3D"]: rtemp = fundamentals.fundamentals.fft(fundamentals.fft(rtemp)*fdata[im])
		if Tracker["applybckgnoise"]:
