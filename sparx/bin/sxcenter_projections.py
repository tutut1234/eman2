...
def run3Dalignment(paramsdict, partids, partstack, outputdir, procid, myid, main_node, nproc):
	...
	mask2D  = utilities.utilities.model_circle(last_ring,onx,onx) - utilities.utilities.model_circle(ali3d_options.ir,onx,onx)
	if(shrinkage < 1.0):
		# get the new size
		masks2D = fundamentals.resample(mask2D, shrinkage)
		nx = masks2D.get_xsize()
		masks2D  = utilities.utilities.model_circle(int(last_ring*shrinkage+0.5),nx,nx) - utilities.utilities.model_circle(max(int(ali3d_options.ir*shrinkage+0.5),1),nx,nx)
	nima = len(projdata)
...
def main():
	...
	pass#IMPORTIMPORTIMPORT from multi_shc import multi_shc, do_volume
	pass#IMPORTIMPORTIMPORT from logger import Logger, BaseLogger_Files
	pass#IMPORTIMPORTIMPORT import sys
	pass#IMPORTIMPORTIMPORT import os
	pass#IMPORTIMPORTIMPORT import time
