	...
	for t in range(maxTsteps):  # Over temperature
		...
		for i in range(maxSteps): # At each temperature, many Monte Carlo steps
			...
			while True: # Will find two random cities sufficiently close by
				...
				n[0] = int((nct)*random.random.rand())     # select one city
				n[1] = int((nct-1)*random.random.rand())   # select another city, but not the same
				if (n[1] >= n[0]): n[1] += 1   #
			...
			if Preverse > random.random.rand(): 
				...
				if de<0 or numpy.exp(-de/T)>random.random.rand(): # Metropolis
					accepted += 1
					dist += de
					reverse(city, n)
			else:
				# Here we transpose a segment
				nc = (n[1]+1+ int(random.random.rand()*(nn-1)))%nct  # Another point outside n[0],n[1] segment. See picture in lecture nodes!
				n[4] = nc
				n[5] = (nc+1) % nct
	...
	if options.dd:
		...
		for i in range(lend-1):
			...
			for j in range(i+1, lend):
				lccc[statistics.mono(i,j)] = [statiststatistics.monoccc(v1, utilities.get_im( stack, j ), mask), 0]
		...
		if  options.pairwiseccc == " " or not os.path.exists(options.pairwiseccc) :
			...
			for i in range(lend-1):
				for j in range(i+1, lend):
					...
					lccc[statistics.mono(i,j)] = [statisticsstatistics.mono(d[j], fundamentals.rot_shift2D(d[i], alpha, sx, sy, mir, 1.0), mask), alpha, sx, sy, mir]
