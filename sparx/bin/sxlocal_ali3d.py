...
def main():
	...
	if(len(args) < 2 or len(args) > 3):
		...
		if options.fourvar:
			...
			if options.MPI:
				sparx_applications.local_ali3d_MPI_scipy_minimization(args[0], args[1], mask, options.ou, options.delta, options.ts, options.center, options.maxit,
				options.CTF, options.snr, options.sym, options.chunk, options.function, options.fourvar,
				options.npad, options.debug)
		else:
			sparx_applications.local_ali3d(args[0], args[1], mask, options.ou, options.delta, options.ts, options.center, options.maxit,
			options.CTF, options.snr, options.sym, options.chunk, options.function, options.fourvar,
