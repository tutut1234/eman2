	...
	for centroid in centroids:
		...
		if gridding == False:
			...
			try:
				...
				ptcl = EMAN2_cppwrap.Util.window( sparx_fundamentals.rot_shift2D(ptcl, -rot_angle, interpolation_method="gridding"), side1, side1, 1, 0, 0, 0)
				ptcl["ptcl_helix_coords"] = tuple(helix_coords)
				ptcl["ptcl_source_coord"] = tuple(centroid)
				ptcl["xform.align2d"]     = EMAN2_cppwrap.Transform()
				ptcl["xform.projection"]  = EMAN2_cppwrap.Transform()
...
def db_save_particles(micrograph_filepath, ptcl_filepath = None, px_dst = None, px_length = None, px_width = None, rotated = True, do_edge_norm = False, gridding = False, stack_file_mode = "multiple", do_filt = True, filt_freq = -1):
	...
	if do_filt:
		...
		micrograph = sparx_filter.filt_gaussh(micrograph, filt_freq)		
	nhelix = 0
	for coords in helices_dict:
		helix = helices_dict[coords]
		if rotated:
...
if ENABLE_GUI:
	...
	#        self.file_menu.addAction(self.write_coords_action)
		...
		def calc_ctf_cter(self):
			...
			img = sparx_utilities.get_im(image_name)
			...
			sparx_utilities.set_ctf(img, [defocus, ctf_cs, ctf_volt, input_pixel_size, 0, ctf_ampcont, ast_amp, ast_agl])
			# and rewrite image 
			...
			if row >= 0: #will be -1 when all rows are removed as first step of self.update_micrograph_table()
				...
				if new_filepath != self.micrograph_filepath:
					...
					if self.invert_contrast:
						...
						sttt = sparx_utilities.info(micrograph)
						avgimg = sparx_utilities.model_blank(mnx, ny=mny, bckg=sttt[0])
						...
						sttt2 = sparx_utilities.info(micrograph)
...
def windowallmic(dirid, micid, micsuffix, outdir, pixel_size, boxsize=256, minseg = 6, outstacknameall='bdb:data', \
	...
	sparx_utilities.print_begin_msg("windowallmic\n")
	...
	for coutdir in outdirlist:
		sparx_utilities.print_msg("Creating output directory %s\n"%coutdir)
		os.mkdir(coutdir)
	...
	for v1 in micdirlist:
		...
		nfiles = len(flist2)
		sparx_utilities.print_msg('Sorted file list in %s:\n'%v1)
		for iii in range(nfiles):
			sparx_utilities.print_msg('%s,'%flist2[iii])
...
def windowmic(outstacknameall, micpath, outdir, micname, hcoordsname, pixel_size, boxsize, ptcl_dst, minseg, inv_contrast, new_pixel_size, rmaxp, freq, do_rotation, do_gridding, importctf, limitctf, cterr, cutoffhistogram):
	...
	if importctf:	
		ctfs = sparx_utilities.read_text_row(importctf)
		nx = True
		...
		if(ctfs[8]/ctfs[0] > cterr[0]):
			sparx_utilities.print_msg('Defocus error %f exceeds the threshold. Micrograph %s rejected.\n'%(ctfs[8]/ctfs[0], filename))
			return
		if(ctfs[10] > cterr[1] ):
...
# 			Cut off frequency components higher than CTF limit 
	...
	if new_pixel_size != pixel_size:
		...
		sparx_utilities.print_msg('Resample micrograph to pixel size %f and window segments from resampled micrograph\n'%new_pixel_size)
		...
		nx = img.get_xsize()
		ny = img.get_ysize()
		img = sparx_fundamentals.resample(img, resample_ratio)
		...
		hcoordsname = sparx_pixel_error.getnewhelixcoords(hcoordsname, outdir, resample_ratio,nx,ny, newpref="resampled_", boxsize=boxsize)
	...
	imgmic  = sparx_utilities.get_im(micname)
	...
	mask = sparx_utilities.pad(sparx_utilities.model_blank(rmaxp*2, boxsize, 1, 1.0), boxsize, boxsize, 1, 0.0)

	a = sparx_utilities.read_text_row(hcoordsname)
	if len(a)%2 != 0:
	...
	#except:   iseg = 0
	if importctf:
		ctfs = sparx_utilities.generate_ctf(ctfs[:8])
	...
	for coords in helices_dict:                                   ## added by@ming
		...
		if( os.path.exists(ptcl_images) ):
			...
			if( n1 < minseg ):
				sparx_utilities.print_msg( "Filament %s has too few segments and was skipped\n"%otcl_images)
			else:
				sparx_utilities.print_msg( "otcl_images: %s\n"%otcl_images)
				sparx_utilities.print_msg( "ptcl_images: %s\n"%ptcl_images)
				for j in range(n1):
					prj = sparx_utilities.get_im(ptcl_images, j)
					prj = sparx_fundamentals.ramp(prj)
