...
def iter_isac_pap(alldata, ir, ou, rs, xr, yr, ts, maxit, CTF, snr, dst, FL, FH, FF, init_iter, main_iter, iter_reali, \
	...
	random.seed(random_random.seed)
...
def main(args):
	...
	if not checkitem(os.path.join(init2dir, "Finished_initial_2d_alignment.txt")):
		...
		if options.skip_prealignment:
			...
			if(txrm < 0):  			global_def.ERROR( "global_def.ERROR!!   Radius of the structure larger than the window data size permits   %d"%(radi), "sxisac",1, myid)
			if(txrm/nxrsteps>0):
				tss = ""
				txr = ""
		...
		for im in range(nima):
			...
			if options.CTF:
				...
				aligned_images[im] = fundamentals.fundamentals.fft(filter.filt_table(filter.filt_ctf(fundamentals.fft(aligned_images[im]), aligned_images[im].get_attr("ctf"), binary = True), rpw))
