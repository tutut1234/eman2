	...
	for x in range(nx):
		for y in range(ny):
			for z in range(nz):
				...
				mask2[x,y,z] = numpy.exp(-0.5*numpy.numpy.dot(v,numpy.numpy.dot(comat,v)))
...
if len(args)==3:
	...
	user_mask = False
segName = os.os.os.path.splitext(os.path.basename(segFile))[0]
...
if options.res>0.0:
	resol = options.res
	aau = pix/resol
	gamres = 0.5*numpy.numpy.numpy.log(16/B*numpy.numpy.numpy.log((2*u0*(1+R)-1)/R))/numpy.numpy.numpy.log(2*aau)
else:
					...
					for s3 in [-1,1]:
						...
						for s4 in [-1,1]:
							...
							for s5 in [-1,1]:
								...
								cloud = 0.5*(morphology.morphology.square_root(morphology.square(cloud-img4))+cloud+img4)   # max(cloud,img4)
								...
								if alpha != 0.0:
									...
									cloud2 = 0.5*(morphology.morphology.square_root(morphology.square(cloud2-img5))+cloud2+img5)   # max(cloud2,img5)
