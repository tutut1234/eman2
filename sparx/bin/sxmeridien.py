...
def calculate_2d_params_for_centering(kwargs):
	...
	if( Finished_initial_2d_alignment == 1 ):
		...
		if(txrm < 0):  			global_def.ERROR( "global_def.ERROR!!   Radius of the structure larger than the window data size permits   %d"%(radi), "sxisac",1, Blockdata["myid"])
		if(txrm/nxrsteps>0):
			tss = ""
			txr = ""
...
def Numrinit_local(first_ring, last_ring, skip=1, mode="F"):
	...
	skip = 1
	if (mode == 'f' or mode == 'F'): dnumpy.pi = 2*numpy.pi
	else:                            dpi = numpy.pi
	numr = []
	lcirc = 1
...
def ali3D_primary_polar(refang, shifts, coarse_angles, coarse_shifts, procid, original_data = None, oldparams = None, \
	...
	for im in range(nima):
		...
		dataimage = fundamentals.fpol(EMAN2_cppwrap.EMAN2_cppwrap.Util.mulnclreal(EMAN2_cppwrap.EMAN2_cppwrap.Util.mulnclreal(dataml, EMAN2_cppwrap.EMAN2_cppwrap.EMAN2_cppwrap.Util.muln_img(bckgn, ctfs)), mask ), Tracker["nxpolar"], Tracker["nxpolar"],1, True)
...
def cerrs(params, ctfs, particle_groups):
	...
	for itry in range(len(params)):
		...
		ctfsbckgnoise = EMAN2_cppwrap.EMAN2_cppwrap.Util.muln_img(EMAN2_cppwrap.Util.unroll1dpw(Tracker["nxinit"], [Blockdata["bckgnoise"][i,particle_groups[itry]] for i in range(lb)]), ct)
...
def refinement_one_iteration(partids, partstack, original_data, oldparams, projdata, general_mode = True, continuation_mode = False):
	...
	if( Blockdata["myid"] == Blockdata["main_node"]):
		...
		pinids = utilities.utilities.read_text_file(partids[0])  + utilities.utilities.read_text_file(partids[1])
		params = utilities.utilities.read_text_row(partstack[0]) + utilities.utilities.read_text_row(partstack[1])
	...
	if(Tracker["mainiteration"] == 1 ):
		...
		if(Blockdata["myid"] == Blockdata["main_node"]):
			params = utilities.utilities.read_text_row(os.path.join(Tracker["directory"],"params-chunk_0_%03d.txt"%(Tracker["mainiteration"])))+utilities.read_text_row(os.path.join(Tracker["directory"],"params-chunk_1_%03d.txt"%(Tracker["mainiteration"])))
			li     = utilities.utilities.read_text_file(os.path.join(Tracker["directory"],"chunk_0_%03d.txt"%(Tracker["mainiteration"])))+utilities.read_text_file(os.path.join(Tracker["directory"],"chunk_1_%03d.txt"%(Tracker["mainiteration"])))
			ctfs   = EMAN2_cppwrap.EMUtil.get_all_attributes(Tracker["constants"]["stack"],'ctf')
			ctfs   = [ctfs[i] for i in li]
			particle_groups = utilities.utilities.read_text_file(os.path.join(Tracker["constants"]["masterdir"],"main000", "particle_groups_0.txt") ) + utilities.utilities.read_text_file(os.path.join(Tracker["constants"]["masterdir"],"main000", "particle_groups_1.txt") )
	...
	if(Blockdata["myid"] == Blockdata["main_node"]):
		anger, shifter = params_changes( utilities.utilities.read_text_row(os.path.join(Tracker["directory"],"params_%03d.txt"%(Tracker["mainiteration"]))), utilities.utilities.read_text_row(os.path.join(Tracker["previousoutputdir"],"params_%03d.txt"%(Tracker["mainiteration"]-1))) )
		...
		if not restart_mode: #<<<-------Fresh run
			...
			if(Blockdata["myid"] == Blockdata["main_node"]):
				...
				if(Tracker["constants"]["mask3D"] == None):
					...
					viv = filter.filt_table(utilities.utilities.get_im(volinit)*utilities.utilities.get_im(Tracker["constants"]["mask3D"]), [1.0]*Tracker["constants"]["inires"] + [0.5] + [0.0]*Tracker["constants"]["nnxo"])
