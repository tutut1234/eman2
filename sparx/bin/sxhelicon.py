...
def main():
	...
	if len(args) < 3 or len(args) > 4:
		...
		if( dp/2.0 < ywobble):
			sparx_global_def.ERROR('ywobble has to be smaller than dp/2.', 'sxhelicon')
			sys.exit()

		try:
			sys.argv = mpi.mpi_init(len(sys.argv), sys.argv)
		except:
			sparx_global_def.ERROR('This program has only MPI version.  Please install MPI library.', 'sxhelicon')
			sys.exit()
		...
		sparx_applications.ehelix_MPI(args[0], args[1], args[2], options.seg_ny, options.delta, options.phiwobble, options.psi_max,\
