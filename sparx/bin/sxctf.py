...
#			for i in range(1,len(snr)): snr[i]/=snr[-1]				# normalize
			...
			inten=[fabs(i) for i numpy.fabstf.compute_1d(len(s)*2,ds,EMAN2_cppwrap.Ctf.CtfType.CTF_AMP)]		# The snr curve
			self.guiplot.set_data((s,inten[:len(s)]),"single",True)
			all=[0 for i in inten]
			for st in self.data:
				print(st)
				inten=[fabs(i) for i numpy.fabst[1].compute_1d(len(s)*2,ds,EMAN2_cppwrap.Ctf.CtfType.CTF_AMP)]
