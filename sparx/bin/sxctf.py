...
#		mask1.process_inplace("mask.decayedge2d",{"width":4})
	...
	pav1 = sparx_utilities.model_blank(ys2,ys2)
	pva1 = sparx_utilities.model_blank(ys2,ys2)
	pav2 = sparx_utilities.model_blank(ys2,ys2)
	pva2 = sparx_utilities.model_blank(ys2,ys2)
...
#		im1=EMData(stackfile,i)
		...
		fofo.append(sparx_fundamentals.rot_avg_table(pp))
	...
	pav1 /= n
	pva1 = sparx_morphology.square_root((pva1 - pav1*pav1*n)/(n-1)/n)
	pav2 /= n
	pva2 = sparx_morphology.square_root((pva2 - pav2*pav2*n)/(n-1)/n)
	pav1.write_image("av1.hdf",0)
	pav2.write_image("av2.hdf",0)
	pva1.write_image("va1.hdf",0)
	pva2.write_image("va2.hdf",0)
	sparx_utilities.write_text_file([list(range(ys2//2+1)), sparx_fundamentals.rot_avg_table(pav1), sparx_fundamentals.rot_avg_table(pva1), sparx_fundamentals.rot_avg_table(pav2), sparx_fundamentals.rot_avg_table(pva2)],"pwsa.txt")
