	...
	for i in range(n):
		...
		if phaseflip :
			if not lctf or not lctf.equal(ctf):
				flipim=fft1.copy()
				ctf.compute_2d_complex(flipim,EMAN2_cppwrap.Ctf.EMAN2_cppwrap.CtfType.CTF_SIGN)
			fft1.mult(flipim)
			out=fft1.do_ift()
			out["ctf"]=ctf
		...
		if wiener :
			if not lctf or not lctf.equal(ctf):
				wienerim=fft1.copy()
				ctf.compute_2d_complex(wienerim,EMAN2_cppwrap.Ctf.EMAN2_cppwrap.CtfType.CTF_WIENER_FILTER)
...
#		im1=EMData(stackfile,i)
	...
	utilities.write_text_file([list(range(ys2//2+1)), fundamentals.fundamentals.fundamentals.fundamentals.rot_avg_table(pav1), fundamentals.fundamentals.fundamentals.fundamentals.rot_avg_table(pva1), fundamentals.fundamentals.fundamentals.fundamentals.rot_avg_table(pav2), fundamentals.fundamentals.fundamentals.fundamentals.rot_avg_table(pva2)],"pwsa.txt")
...
def ctf_fit(im_1d,bg_1d,im_2d,bg_2d,voltage,cs,ac,apix,bgadj=0,autohp=False):
	...
	for dfi in range(5,128):			# loop over defocus
		...
		ctf.defocus=df
		ctf.ampcont=ac
		cc=ctf.compute_1d(ys,ds,EMAN2_cppwrap.Ctf.EMAN2_cppwrap.CtfType.CTF_AMP)
		st=.04/ds
		norm=0
...
#		self.app.app.closeAllWindows()
	...
	def update_plot(self):
		...
		if self.plotmode==1:
			...
			fit=ctf.compute_1d(len(s)*2,ds,EMAN2_cppwrap.Ctf.EMAN2_cppwrap.CtfType.CTF_AMP)		# The fit curve
			...
			snr=ctf.compute_1d(len(s)*2,ds,EMAN2_cppwrap.Ctf.EMAN2_cppwrap.CtfType.CTF_SNR)		# The snr curve
			self.guiplot.set_data((s,snr[:len(s)]),"snr",True)
			self.guiplot.setAxisParms("s (1/A)","SNR (intensity ratio)")
		elif self.plotmode==3:
			snr=ctf.compute_1d(len(s)*2,ds,EMAN2_cppwrap.Ctf.EMAN2_cppwrap.CtfType.CTF_SNR)		# The snr curve
			self.guiplot.set_data((s,snr[:len(s)]),"snr",True)
			ssnr=ctf.compute_1d(len(s)*2,ds,EMAN2_cppwrap.Ctf.EMAN2_cppwrap.CtfType.CTF_SNR_SMOOTH)		# The fit curve
...
#			for i in range(1,len(snr)): snr[i]/=snr[-1]				# normalize
			...
			inten=[numpy.fabs(i) for i in ctf.compute_1d(len(s)*2,ds,EMAN2_cppwrap.Ctf.EMAN2_cppwrap.CtfType.CTF_AMP)]		# The snr curve
			self.guiplot.set_data((s,inten[:len(s)]),"single",True)
			all=[0 for i in inten]
			for st in self.data:
				print(st)
				inten=[numpy.fabs(i) for i in st[1].compute_1d(len(s)*2,ds,EMAN2_cppwrap.Ctf.EMAN2_cppwrap.CtfType.CTF_AMP)]
