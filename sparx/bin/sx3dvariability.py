...
def main():
	...
	#####
	pass#IMPORTIMPORTIMPORT from mpi import mpi_init, mpi_comm_rank, mpi_comm_size, mpi_recv, MPI_COMM_WORLD
	...
	pass#IMPORTIMPORTIMPORT from reconstruction import recons3d_em, recons3d_em_MPI
	...
	pass#IMPORTIMPORTIMPORT from utilities import read_text_row, get_image, get_im
	pass#IMPORTIMPORTIMPORT from utilities import bcast_EMData_to_all, bcast_number_to_all
	...
	if options.symmetrize :
		...
		if options.VAR: # 2D variance images have no shifts
			...
			pass#IMPORTIMPORTIMPORT from utilities		import set_params_proj, get_params_proj, params_3D_2D, get_params2D, set_params2D, compose_transform2
			pass#IMPORTIMPORTIMPORT from utilities		import model_blank, nearest_proj, model_circle
			pass#IMPORTIMPORTIMPORT from applications	import pca
			pass#IMPORTIMPORTIMPORT from statistics		import avgvar, avgvar_ctf, ccc
			pass#IMPORTIMPORTIMPORT from filter		    import filt_tanl
			pass#IMPORTIMPORTIMPORT from morphology		import threshold, square_root
			pass#IMPORTIMPORTIMPORT from projection 	import project, prep_vol, prgs
				...
				if options.fl > 0.0:
					if options.CTF:
						for k in range(img_per_grp):
							...
							cimage = fundamentals.fundamentals.fft(filter.filt_tanl(filter.filt_ctf(fundamentals.fft(utilities.pad(grp_imgdata[k], nx2, ny2, 1,0.0)),\
					...
					if options.CTF:
						for k in range(img_per_grp):
							cimage = fundamentals.fundamentals.fft(filter.filt_ctf(fundamentals.fft(utilities.pad(grp_imgdata[k], nx2, ny2, 1,0.0)),\
