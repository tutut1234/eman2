...
for ii in range(1,696+1) :
	...
	selected_particles = sparx_utilities.read_spider_doc(fn_high)

	nimage  = EMAN2_cppwrap.EMUtil.get_image_count( proj_in )
	trans = sparx_utilities.read_text_row(fn_tran, format = "s")
	angl  = sparx_utilities.read_text_row(fn_angl, format = "s")
	...
	for iq in range(len(selected_particles)):
		...
		assb, sxnb, synb, sc = sparx_utilities.compose_transform2(0.0, sx, sy, 1,  -alpha, 0.,0.,1)
		if( mir > 0.5 ) :
			phi   = (540.0 + phi)%360.0
			theta = 180.0 - theta
		...
		deci = data
		sparx_utilities.set_params_proj(deci, [phi, theta, psi, sxnb, synb])
		...
		sparx_utilities.set_ctf(deci, [defocus, Cs, voltage, pixel, bfactor, amp_cont])
