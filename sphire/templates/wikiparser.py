...
class SXkeyword_map(object):
	def __init__(self, priority, token_type):
		if priority >= 100: sxglobal_def.ERROR("Priority should be lower than 100", "%s in %s" % (__name__, os.path.basename(__file__)))
		# ><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><
		# class variables
...
def remove_MoinMoinWiki_makeup(target_text):
	...
	while item_head != -1:
		...
		if item_tail == -1:
			sxglobal_def.ERROR("Wiki Format Warning: The string \"%s\" contains \"%s\" but not \"%s\". Removing \"%s\", but please check the format in Wiki document." % (target_text, makeup_begin, makeup_end, makeup_begin), "%s in %s" % (__name__, os.path.basename(__file__)))
...
def construct_token_list_from_MoinMoinWiki(sxcmd_config):
	...
	if sxcmd_config.format != "MoinMoinWiki": sxglobal_def.ERROR("Logical Error: Incorrect Wiki format %s! Check the sxcmd_config setting in this script." % (sxcmd_config.format), "%s in %s" % (__name__, os.path.basename(__file__)))
	...
	if os.path.exists(sxcmd_config.wiki) == False: sxglobal_def.ERROR("Rutime Error: Wiki document is not found.", "%s in %s" % (__name__, os.path.basename(__file__)))
	...
	for line_wiki in file_wiki:
		...
		if current_state == state_searching:
			...
			if current_state != state_processing: sxglobal_def.ERROR("Logical Error: This condition should not happen! State setting must be incorrect.", "%s in %s" % (__name__, os.path.basename(__file__)))
			if line_wiki[0] == "=": # Assuming the section always starts with "="
				...
				if line_wiki.find(section_lists[current_section]) != -1:
					# Found the current target section
					if current_section >= len(section_lists): sxglobal_def.ERROR("Logical Error: This condition should not happen! Section setting must be incorrect.", "%s in %s" % (__name__, os.path.basename(__file__)))
					current_state = state_processing
				else:
				...
				if current_section == section_name:
					...
					if item_tail == -1: sxglobal_def.ERROR("Wiki Format Error: '= Name =' section should contain only one valid line, and the line should starts from 'sx* - ' or 'e2* - ': %s" % line_wiki, "%s in %s" % (__name__, os.path.basename(__file__)))
					...
					if n_sxcmd_string_token_list == 2:
						...
						sxglobal_def.ERROR("Wiki Format Error: Command string should be only script file name or script file name plus subcommand", "%s in %s" % (__name__, os.path.basename(__file__)))
					line_buffer = line_buffer[item_tail + len(target_operator):].strip() # Get the rest of line
					# Extract the label of this sxscript
					target_operator = ":"
					item_tail = line_buffer.find(target_operator)
					if item_tail == -1: sxglobal_def.ERROR("Wiki Format Error: '= Name =' section should contain a label ended with ':' after 'sx* - ' or 'e2* - ': %s" % line_wiki, "%s in %s" % (__name__, os.path.basename(__file__)))
					...
					if line_wiki[0:len("sx")] == "sx" or line_wiki[0:len("e2")] == "e2":
						...
						head_token_idx = 1
						if usage_token_list[0] != sxcmd.name + ".py": sxglobal_def.ERROR("Wiki Format Error: First token should be script name with .py (sx*.py or e2*.py)", "%s in %s" % (__name__, os.path.basename(__file__)))
						if sxcmd.subname != "":
							head_token_idx = 2
							if usage_token_list[1] != sxcmd.subname: sxglobal_def.ERROR("Wiki Format Error: Second token of this command should be subname", "%s in %s" % (__name__, os.path.basename(__file__)))
					...
					if line_wiki[0] == "*" and line_wiki.find("optional"):
						...
						if key_base == "MPI":
							...
							if sxcmd.mpi_support == False or sxcmd.mpi_add_flag == False: sxglobal_def.ERROR("Logical Error: Since MPI flag is found, the command should support MPI.", "%s in %s" % (__name__, os.path.basename(__file__)))
							continue
						line_buffer = line_buffer[item_tail + len(target_operator):].strip() # Get the rest of line
						# check consistency between 'usage in command line' and this
						if key_base not in list(sxcmd.token_dict.keys()): sxglobal_def.ERROR("Wiki Format Error: Key base (%s) is missing from 'usage in command line' in '= Usage ='." % key_base, "%s in %s" % (__name__, os.path.basename(__file__)))
						# Get the reference to the command token object associated with this key base name
						token = sxcmd.token_dict[key_base]
						if token.key_base != key_base: sxglobal_def.ERROR("Logical Error: Registered command token with wrong key base name into the dictionary.", "%s in %s" % (__name__, os.path.basename(__file__)))
						...
						target_operator = ":"
						item_tail = line_buffer.find(target_operator)
						if item_tail == -1: sxglobal_def.ERROR("Wiki Format Error: This line (%s) is missing label. Please check the format in Wiki document." % line_wiki, "%s in %s" % (__name__, os.path.basename(__file__)))
						token.label = line_buffer[0:item_tail]
						line_buffer = line_buffer[item_tail + len(target_operator):].strip() # Get the rest of line
						# Extract help of command token before default value
						target_operator = "(default"
						item_tail = line_buffer.find(target_operator)
						if item_tail == -1: sxglobal_def.ERROR("Wiki Format Error: This line (%s) is missing default setting. Please check the format in Wiki document." % line_wiki, "%s in %s" % (__name__, os.path.basename(__file__)))
						token.help = remove_MoinMoinWiki_makeup(line_buffer[0:item_tail])
						line_buffer = line_buffer[item_tail + len(target_operator):].strip() # Get the rest of line
						# Extract default value of command token
						target_operator = ")"
						item_tail = line_buffer.find(target_operator)
						if item_tail == -1: sxglobal_def.ERROR("Wiki Format Error: This line (%s) is missing ')' for default setting. Please check the format in Wiki document." % line_wiki,"%s in %s" % (__name__, os.path.basename(__file__)))
					...
					sxglobal_def.ERROR("Logical Error: This section is invalid. Did you assign an invalid section?", "%s in %s" % (__name__, os.path.basename(__file__)))

	if current_state != state_done: sxglobal_def.ERROR("Wiki Format Error: parser could not extract all information necessary (current state %d). Please check if the Wiki format has all required sections." % (current_state), "%s in %s" % (__name__, os.path.basename(__file__)))

	# Make sure there are no extra arguments or options in 'usage in command line' of '= Usage ='
	for token in sxcmd.token_list:
		if token.is_in_io == False: sxglobal_def.ERROR("Wiki Format Error: An extra argument or option (%s) is found in 'usage in command line' of '= Usage ='." % token.key_base, "%s in %s" % (__name__, os.path.basename(__file__)))
...
def construct_token_list_from_DokuWiki(sxcmd_config):
	...
	if sxcmd_config.format != "DokuWiki": sxglobal_def.ERROR("Logical Error: Incorrect Wiki format %s! Check the sxcmd_config setting in this script." % (sxcmd_config.format), "%s in %s" % (__name__, os.path.basename(__file__)))
	...
	# NOTE: 2015/11/11 Toshio Moriya
	# This should be exception. Need to decide if this should be skipped or exit system.
	if os.path.exists(sxcmd_config.wiki) == False: sxglobal_def.ERROR("Runtime Error: Wiki document inot found.", "%s in %s" % (__name__, os.path.basename(__file__)))
...
#				print("MRK_DEBUG: line_wiki[-len(target_operator):] := %s" % (line_wiki[-len(target_operator):]))
				sxglobal_def.ERROR("Wiki Format Error: The Wiki document must start from header section title defined by ===== COMMAND_NAME =====.", "%s in %s" % (__name__, os.path.basename(__file__)))
			...
			if n_sxcmd_string_token_list == 2:
				...
				sxglobal_def.ERROR("Wiki Format Error: Command string should be only a script file name or a script file name plus a subcommand", "%s in %s" % (__name__, os.path.basename(__file__)))
			...
			target_operator = ":"
			item_tail = line_buffer.find(target_operator)
			if item_tail == -1: sxglobal_def.ERROR("Wiki Format Error: Header section body should contain a label ending with ':': %s" % line_wiki, "%s in %s" % (__name__, os.path.basename(__file__)))
			sxcmd.label = line_buffer[0:item_tail].strip()
			# Extract the short info about this sxscript (can be empty)
...
#				print("MRK_DEBUG: New section_lists[current_section] := %s" % (section_lists[current_section]))
				...
				if current_section == section_usage:
					...
					if line_wiki[0:len("sx")] == "sx" or line_wiki[0:len("e2")] == "e2":
						...
						if usage_token_list_line[0] != sxcmd.name + ".py": sxglobal_def.ERROR("Wiki Format Error: First token should be script name with .py (sx*.py or e2*.py)", "%s in %s" % (__name__, os.path.basename(__file__)))
						if sxcmd.subname != "":
							head_token_idx = 2
							if usage_token_list_line[1] != sxcmd.subname: sxglobal_def.ERROR("Wiki Format Error: Second token of this command should be subname", "%s in %s" % (__name__, os.path.basename(__file__)))
						# Register arguments and options
					...
					if line_wiki.find(group_lists[group_advanced]) > -1:
						...
						if key == "--MPI":
							# ERROR("Warning: This line (%s) contains MPI flag. The flag will be removed in near future, so ignoring this line...'."  % (line_wiki), "%s in %s" % (__name__, os.path.basename(__file__)), action = 0)
							if sxcmd.mpi_support == False: sxglobal_def.ERROR("Logical Error: Since MPI flag is found, the command should support MPI.", "%s in %s" % (__name__, os.path.basename(__file__)))
							sxcmd.mpi_add_flag = True
							continue
						# Allocate memory for new command token
						token = sxgui_template.SXcmd_token()
...
###						token.is_in_io = True # Set flag to tell this command token is found in input section
					...
					sxglobal_def.ERROR("Logical Error: This section is invalid. Did you assigne an invalid section?", "%s in %s" % (__name__, os.path.basename(__file__)))
	...
	for sxcmd_token_key_base in list(sxcmd.token_dict.keys()):
		...
		for sxcmd_token in sxcmd.token_list:
			if sxcmd_token.key_base == sxcmd_token_key_base:
				is_found = True
				break
		if not is_found: sxglobal_def.ERROR("Logical Error: Registered key base name in the command token dictionary is not found in the command token list.", "%s in %s" % (__name__, os.path.basename(__file__)))
		...
		for usage_token in usage_token_list:
			if usage_token.key_base == sxcmd_token_key_base:
				...
				is_found = True
				break
		if not is_found: sxglobal_def.ERROR("Wiki Format Error: An extra argument or option (%s) is found in the command token dictionary extracted from '===== Input =====' compared with 'usage in command line' of '====== Usage ======'." % sxcmd_token_key_base, "%s in %s" % (__name__, os.path.basename(__file__)))

	for sxcmd_token in sxcmd.token_list:
		# Make sure there are no extra arguments or options in command token list compared with command token dictionary.
		if sxcmd_token.key_base not in list(sxcmd.token_dict.keys()): sxglobal_def.ERROR("Logical Error: Registered key base name in the command token list is not registered as key base name in the command token dictionary.", "%s in %s" % (__name__, os.path.basename(__file__)))
		# Make sure there are no extra arguments or options in command token list compared with usage token list extracted from "usage in command line" of "====== Usage ======".
		is_found = False
		for usage_token in usage_token_list:
			if usage_token.key_base == sxcmd_token.key_base:
				...
				is_found = True
				break
		if not is_found: sxglobal_def.ERROR("Wiki Format Error: An extra argument or option (%s) is found in the command token list extracted from '===== Input =====' compared with 'usage in command line' of '====== Usage ======'." % sxcmd_token.key_base, "%s in %s" % (__name__, os.path.basename(__file__)))
	...
	for usage_token in usage_token_list:
		...
		if usage_token.key_base not in list(sxcmd.token_dict.keys()): sxglobal_def.ERROR("Wiki Format Error: An extra argument or option (%s) is found in 'usage in command line' of '====== Usage ======' compared with the command token dictionary extracted from '===== Input =====' ." % usage_token.key_base, "%s in %s" % (__name__, os.path.basename(__file__)))
		...
		for sxcmd_token in sxcmd.token_list:
			if sxcmd_token.key_base == sxcmd_token_key_base:
				...
				is_found = True
				break
		if not is_found: sxglobal_def.ERROR("Wiki Format Error: An additional argument or option (%s) is found in 'usage in command line' of '====== Usage ======' compared with the command token list extracted from '===== Input =====' ." % usage_token.key_base, "%s in %s" % (__name__, os.path.basename(__file__)))
...
def apply_sxsubcmd_config(sxsubcmd_config, sxcmd):
	...
	if mode_token_edit.key_base not in list(fullset_token_dict.keys()): sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. Key (%s) should exists." % (mode_token_edit.key_base), "%s in %s" % (__name__, os.path.basename(__file__)))
	...
	for token_edit in sxsubcmd_config.token_edit_list:
		...
		if token_edit.key_base is None: sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. Invalid None Key (%s)." % (token_edit.key_base) , "%s in %s" % (__name__, os.path.basename(__file__)))
		if token_edit.key_base == "": sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. Invalid empty string Key (%s)." % (token_edit.key_base) , "%s in %s" % (__name__, os.path.basename(__file__)))
		...
		if token_edit.key_base not in list(fullset_token_dict.keys()):
			...
			if token_edit.key_prefix is None: sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. Prefix (%s) for Key (%s) should NOT be None." % (token_edit.key_prefix, token_edit.key_base) , "%s in %s" % (__name__, os.path.basename(__file__)))
			if token_edit.key_prefix != "": sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. Key (%s) should be argument (Prefix (%s) should be empty string)." % (token_edit.key_base, token_edit.key_prefix) , "%s in %s" % (__name__, os.path.basename(__file__)))
			# token = token_edit
			token = sxgui_template.SXcmd_token()
		...
		if token.key_base is None: sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.key_base should NOT be None.", "%s in %s" % (__name__, os.path.basename(__file__)))
		if token.key_prefix is None: sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.key_prefix should NOT be None.", "%s in %s" % (__name__, os.path.basename(__file__)))
		if token.label is None: sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.label should NOT be None.", "%s in %s" % (__name__, os.path.basename(__file__)))
		if token.help is None: sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.help should NOT be None.", "%s in %s" % (__name__, os.path.basename(__file__)))
		if token.group is None: sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.group should NOT be None.", "%s in %s" % (__name__, os.path.basename(__file__)))
		if token.is_required is None: sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.is_required should NOT be None.", "%s in %s" % (__name__, os.path.basename(__file__)))
		if token.is_locked is None: sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.is_locked should NOT be None.", "%s in %s" % (__name__, os.path.basename(__file__)))
		if token.is_reversed is None: sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.is_reversed should NOT be None.", "%s in %s" % (__name__, os.path.basename(__file__)))
		if token.default is None: sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.default should NOT be None.", "%s in %s" % (__name__, os.path.basename(__file__)))
		if token.restore is None: sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.restore should NOT be None.", "%s in %s" % (__name__, os.path.basename(__file__)))
		if token.type is None: sxglobal_def.ERROR("Logical Error: This condition should not happen! Subset command configuration must be incorrect. token.type should NOT be None.", "%s in %s" % (__name__, os.path.basename(__file__)))
...
def main(is_dev_mode = False, is_MoinMoinWiki_mode = False):
	...
	for sxcmd_config in sxcmd_config_list:
		if not sxcmd_config.category in sxcmd_category_names:
			sxglobal_def.ERROR("Logical Error: sxcmd_config for %s is using invalid category %s." % (sxcmd_config.wiki, sxcmd_config.category), "%s in %s" % (__name__, os.path.basename(__file__)))
...
#	output_file_path = "../bin/sxgui.py"
	...
	for line in sxgui_template_file:
		...
		if current_state == state_template:
			if line.find("# @@@@@ START_INSERTION @@@@@") != -1:
				...
				for sxcmd_config in sxcmd_config_list:
					...
					if sxcmd_config.format == "MoinMoinWiki":
						...
						sxglobal_def.ERROR("Logical Error: Invalid Wiki format %s! Check the sxcmd_config setting in this script." % (sxcmd_config.format), "%s in %s" % (__name__, os.path.basename(__file__)))
			...
			if current_state != state_insertion: sxglobal_def.ERROR("Logical Error: This condition should not happen! State setting must be incorrect.", "%s in %s" % (__name__, os.path.basename(__file__)))
			if line.find("# @@@@@ END_INSERTION @@@@@") != -1:
				current_state = state_template
			# else: do nothing

	if current_state == state_insertion: sxglobal_def.ERROR("Script Template Format Error: START_INSERTION and END_INSERTION must be paired.", "%s in %s" % (__name__, os.path.basename(__file__)))
